...
// Refresh
	...
	function ClockPFD() {
		...
		function RefreshPFDTechInfo() {
			...
			if(Math.abs(PFD0.RawData.Accel.SpeedVector.X) > 0) {
				NeedleAngle = Math.atan(Math.abs(PFD0.RawData.Accel.SpeedVector.Y / PFD0.RawData.Accel.SpeedVector.X)) / (Math.PI / 180);
			} else {
				NeedleAngle = 90;
			}
